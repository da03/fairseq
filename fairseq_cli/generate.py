...
def main(args):
    ...
    if args.results_path is not None:
        ...
        for rank in range(args.ngpus):
            p = Process(target=_main, args=(args, sys.stdout, rank, world_size))
            p.start()
            processes.append(p)
        for p in processes:
            p.join()
...
def _main(args, output_file, rank, world_size, backend='NCCL'):
    ...
    with torch.cuda.device(rank):
        ...
        for sample in progress:
            ...
            gen_timer.start()
            hypos = task.inference_step(generator, models, sample, prefix_tokens, rank=rank, ngpus=world_size)
